define(
		[
				...
				"./CommonScan",
				"dojo/text!./templates/AddDMSContentItemWithScanDialog.html" ],
			...
			return declare(
					...
					[ AddContentItemDialog ],
					{
						...
						onAdd : function() {
							...
							var type=contentItemGeneralPane.getContentSourceType();
							if(type != "Scan"){
								this.inherited(arguments);
								return;
							}
...
//									objectStore = (parentFolder && parentFolder.objectStore);
								...
								// console.log(Request._security_token);
								...
								DWObject.SetHTTPFormField(
										"childComponentValues", dojojson
												.toJson(childComponentValues));
								DWObject.SetHTTPFormField("acl", dojojson
										.toJson(permissions));
								DWObject.SetHTTPFormField("desktop", desktop);
								DWObject.SetHTTPFormField("repositoryId",
										this.repository.id);
								DWObject.SetHTTPFormField("docid", docId);
								DWObject.SetHTTPFormField(
										"parm_content_source_type",
										contentSourceType);
								DWObject.SetHTTPFormField("template_name",
										documentType);
						...
						initScanActionsWithEnteryTemplate : function(repository, items, callback, teamspace, resultSet, parameterMap) {
							...
							contentItemGeneralPane._updateUIForSourceType = function(sourceType) {
								...
								if (sourceType == "Item") {
									...
									domStyle.set(dom.byId("container"),"display", "none");
									thisObj._actionButton.set("disabled",false);
									thisObj._actionButton._setLabelAttr(Messages.add_document);
									domStyle.set(thisObj.scanButton.domNode, "display", "none");
									//thisObj.scanButton.destroy();
								} else if (sourceType == "ExternalURL") {
									domStyle.set(contentItemGeneralPane._fileInputArea, "display", "none");
									domStyle.set(contentItemGeneralPane._externalURLDiv,"display", "");
									...
									thisObj._actionButton._setLabelAttr(Messages.add_document_lable);
									
									if(thisObj.scanButton)
									domStyle.set(thisObj.scanButton.domNode, "display", "block");
							...
							contentItemGeneralPane.onContentSourceTypeChange();
							 ...
							 aspect.after(this.addContentItemPropertiesPane, "onCompleteRendering", lang.hitch(this, function() {
								...
								//console.log(this.addContentItemGeneralPane._documentOnlyArea)
...
//								Dynamsoft.WebTwainEnv.Load();
							...
							if (items != null && items[0].isFolder())
								...
								this.workItemWidget = parameterMap.widget;
								this.show(repository, items[0], true);
							}else if(resultSet.parentFolder != null && (resultSet.parentFolder).isInstanceOf(ecm.model.AttachmentItem)){
