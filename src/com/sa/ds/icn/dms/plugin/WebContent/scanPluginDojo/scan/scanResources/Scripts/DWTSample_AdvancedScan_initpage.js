...
var _iLeft, _iTop, _iRight, _iBottom; // These variables are used to remember
// the selected area
...
function InitMessageBody() {
	...
	if (MessageBody) {
		var ObjString = "<div class='divinput' style='line-height: 18px;border-radius: 4px;color: #30768D;'><b style='color: #1E586B;'>Note:</b> This online demo application uses JavaScript + ASP.NET (C#). If you use other server-side language (JSP, PHP, ASP.NET (VB.NET), ASP etc.), you can visit the ";
...
function initMessageBox(bNeebBack) {
	...
	// objString += "<div style='text-align:center; display:block'>";
	// objString += "<div style='position:relative; float:left; width:422px;
	// height:35px;'>";
	// objString += "<input id='DW_btnFirstImage'
	// onclick='btnFirstImage_onclick()' type='button'
	// class='scanFirst'/>&nbsp;";
	// objString += "<input id='DW_btnPreImage' onclick='btnPreImage_onclick()'
	// type='button' class='scanPrevious'/>&nbsp;&nbsp;";
	// objString += "<input type='text' size='2' id='DW_CurrentImage'
	// readonly='readonly'/>/";
	// objString += "<input type='text' size='2' id='DW_TotalImage'
	// readonly='readonly'/>&nbsp;&nbsp;";
	// objString += "<input id='DW_btnNextImage'
	// onclick='btnNextImage_onclick()' type='button' class='scanNext'/>&nbsp;";
	// objString += "<input id='DW_btnLastImage'
	// onclick='btnLastImage_onclick()' type='button' class='scanLast'/></div>";
	// objString += "<div style='position:relative; float:left; width:150px;
	// height:35px;' id='preview_mode'>Preview Mode";
	// objString += "<select size='1' id='DW_PreviewMode' onchange
	// ='setlPreviewMode();'>";
	// objString += " <option value='0'>1X1</option>";
	// objString += "</select><br /></div>";
	// objString += "<div><input id='DW_btnRemoveCurrentImage'
	// onclick='btnRemoveCurrentImage_onclick()' type='button' value='Remove
	// Selected Images'/>";
	// if (bNeebBack) {
	// objString += "<input id='DW_btnRemoveAllImages'
	// onclick='btnRemoveAllImages_onclick()' type='button' value='Remove All
	// Images'/><br /><br />";
	// objString += "<span style=\"font-size:larger\"><a href
	// =\"online_demo_list.aspx\"><b>Back</b></a></span><br /></div>";
	// } else {
	// objString += "<input id='DW_btnRemoveAllImages'
	// onclick='btnRemoveAllImages_onclick()' type='button' value='Remove All
	// Images'/><br /></div>";
	...
	// objString += "<div id='DWTdivMsg' style='width:580px;display:none;'>";
	// objString += "Message:<br/>"
	// objString += "<div id='DWTemessage' style='display:none;width:560px;
	// padding:30px 0 0 3px; height:20px; margin-top:5px; overflow:auto;
	// border:1px #303030; border-style:solid; position:relative' >";
	// objString += "</div></div>";

	// var DWTemessageContainer =
	// document.getElementById("DWTemessageContainer");
...
function initCustomScan() {
	...
	//    
	// ObjString = "";
	// ObjString += "<ul id='divTwainType' style='height:70px; '> ";
	// ObjString += "<li style='padding-left:12px;'>";
	// ObjString += "<label id ='lblShowUI' for = 'ShowUI'><input
	// type='checkbox'
	// id='ShowUI' />Show UI&nbsp;</label>";
	// ObjString += "<label for = 'ADF'><input type='checkbox' id='ADF'
	// />AutoFeeder&nbsp;</label>";
	// ObjString += "<label for = 'Duplex' id='DuplexLbl'><input type='checkbox'
	// id='Duplex'/>Duplex</label></li>";
	// ObjString += "<li style='padding-left:15px;' id='Pixel_Type'>Pixel
	// Type:";
	// ObjString += "<label for='BW'><input type='radio' id='BW'
	// name='PixelType'/>B&amp;W </label>";
	// ObjString += "<label for='Gray'><input type='radio' id='Gray'
	// name='PixelType'/>Gray</label>";
	// ObjString += "<label for='RGB'><input type='radio' id='RGB'
	// name='PixelType'/>Color</label></li>";
	// ObjString += "<li style='padding-left:15px;'>";
	// ObjString += "<label for='Resolution'>Resolution:<select size='1'
	// id='Resolution'><option value = ''></option></select></label></li>";
	// ObjString += "</ul>";
	//    
...
function setDefaultValue() {
	...
	var imgTypepdf2 = document.getElementById("imgTypepdf2");
	if (imgTypepdf2)
		imgTypepdf2.checked = true;
	var imgTypepdf = document.getElementById("imgTypepdf");
	if (imgTypepdf)
		imgTypepdf.checked = true;
...
function DWTonReady() {
	...
	// If the ErrorCode is 0, it means everything is fine for the control. It is
	// fully loaded.
		...
		if (DWObject.ErrorCode == 0) {
			...
			if (webcamsource) {
				...
				for ( var i = 0; i < vWebcamCount; i++)
					webcamsource.options.add(new Option(arySource[i], i)); // Get
				// Webcam Source names and put  them in a drop-down  box
			...
			if (document.getElementById("ddl_barcodeFormat")) {
				for ( var index = 0; index < BarcodeInfo.length; index++)
					document.getElementById("ddl_barcodeFormat").options
							.add(new Option(BarcodeInfo[index].desc, index));
...
function initCheckIn() {
	//var strHostIP = document.getElementById('strHostIP').value;
	var strHostIP = location.protocol + "//" + document.getElementById("strHostIP").value;
	var intPort = document.getElementById('intPort').value;
	var downloadURL = document.getElementById('downloadURL').value;

	var mType = document.getElementById('mType').value;
	console.log("initCheckIn>>>>>>>>>>>>>>>>>>>>");
	console.log(mType);
	console.log(downloadURL);
	if (downloadURL != '') {
		if (mType == 'image/jpeg' || mType == 'image/jpg') {
			var varImgTypeJpeg = document.getElementById('imgTypejpeg2');
			if (varImgTypeJpeg)
				varImgTypeJpeg.checked = true;
		} else if (mType == 'application/pdf') {
			var varImgTypePdf = document.getElementById('imgTypepdf2');
			console.log("varImgTypePdf: " + varImgTypePdf)
			if (varImgTypePdf)
				varImgTypePdf.checked = true;
		} else if (mType == 'image/png') {
			var varImgTypePng = document.getElementById('imgTypepng2');
			if (varImgTypePng)
				varImgTypePng.checked = true;
		} else {
			var varImgTypeTiff = document.getElementById('imgTypetiff2');
			if (varImgTypeTiff)
				varImgTypeTiff.checked = true;
		}
		var sFun = function() {
			console.log('Success');
		};
		...
		var x = DWObject.HTTPDownload(strHostIP, downloadURL, sFun, fFun);
		if (!x)
			console.log("HTTPDownload Error = " + DWObject.ErrorString);
...
function showtblLoadImage_onclick() {
	...
	if (document.getElementById("pNoScanner")) {
		document.getElementById("tblLoadImage").style.top = ds_gettop(document
				.getElementById("pNoScanner"))
				+ pNoScanner.offsetHeight + "px";
		document.getElementById("tblLoadImage").style.left = ds_getleft(document
				.getElementById("pNoScanner"))
				+ 0 + "px";
	}
	return false;
...
function appendMessage(strMessage) {
	/*
	 * _strTempStr += strMessage; var _divMessageContainer =
	 * document.getElementById("DWTemessage"); if (_divMessageContainer) {
...
function checkErrorString() {
	return checkErrorStringWithErrorCode(DWObject.ErrorCode,
			DWObject.ErrorString);
}

function checkErrorStringWithErrorCode(errorCode, errorString, responseString) {
	if (errorCode == 0) {
		appendMessage("<span style='color:#cE5E04'><b>" + errorString
				+ "</b></span><br />");
