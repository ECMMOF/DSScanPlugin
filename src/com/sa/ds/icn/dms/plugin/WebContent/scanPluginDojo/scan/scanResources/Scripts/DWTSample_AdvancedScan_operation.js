...
function btnUpload_onclick() {
    ...
    if (strImageType == 2 && document.getElementById("MultiPageTIFF_save").checked) {
        if ((DWObject.SelectedImagesCount == 1) || (DWObject.SelectedImagesCount == DWObject.HowManyImagesInBuffer)) {
    ...
    else if (strImageType == 4 && document.getElementById("MultiPagePDF_save").checked) {
    if ((DWObject.SelectedImagesCount == 1) || (DWObject.SelectedImagesCount == DWObject.HowManyImagesInBuffer)) {
...
function rdTIFF_onclick() {
    var _chkMultiPageTIFF = document.getElementById("MultiPageTIFF_save");
    _chkMultiPageTIFF.disabled = false;
    _chkMultiPageTIFF.checked = false;

    var _chkMultiPagePDF = document.getElementById("MultiPagePDF_save");
...
function btnMoveUpCurrentImage_onclick(){
	var index = DWObject.GetSelectedImageIndex(0);
	if (!checkIfImagesInBuffer() && DWObject.SelectedImagesCount >1 &&  (index-1) <0) {
        return;
    }
	DWObject.SwitchImage(index, index-1);
	DWObject.SetSelectedImageIndex(0,--index);
	updatePageInfo();
}

function btnMoveDownCurrentImage_onclick(){
	var index = DWObject.GetSelectedImageIndex(0);
	if (!checkIfImagesInBuffer() && DWObject.SelectedImagesCount >1 &&  (index+1) > (DWObject.SelectedImagesCount-1)) {
        return;
    }
	DWObject.SwitchImage(index, index+1);
	DWObject.SetSelectedImageIndex(0,++index);
