define([
	...
	"dojo/text!./templates/AddContentItemCustomDialog.html",
	"dojo/i18n!./nls/localization",
	"scanPluginDojo/scan/Toaster",
],
...
Toaster) {
	 ...
	 */
	return declare("scanPluginDojo.scan.AddContentItemCustomDialog", [
		BaseDialog
	], {
		...
		postCreate: function() {
			...
			dojo.destroy(this.addContentItemGeneralPane._contentSourceTypeDiv);
		},
		
//		findFolder: function() {
//			var folderNum = this.folderNumberField.get("value");
//			if (!folderNum || folderNum.trim() == "") {
//				this.toaster.redToaster(lcl.FOLDER_NUM_REQUIRED);
//				return;
//			}
//			
//			var className = this.addContentItemPropertiesPane._contentClassSelector.selectedContentClass;
//			
//			var params = {
//					method: "LookupFolderByBarcodeValue",
//					barcodeValue: folderNum,
//					userId: ecm.model.Desktop.userId,
//					className: className.id
//			};
//			
//			var response = ecm.model.Request.invokeSynchronousPluginService("PhysicalArchivePlugin","PhysicalArchiveService",params);
//			var resultSet = new ecm.model.ResultSet(response);
//			if(!resultSet.result.startsWith("ERROR")){
//				this.selectedFolder = JSON.parse(resultSet.result);								
//				document.getElementById("findFolderResult").innerHTML = this.selectedFolder.folderPath.replace("|  |  |  |  |", " | ");
//			} else {
//				var error = resultSet.result;
//				if (error.includes("Error getting Classificafion with symbolic name")) {
//					this.toaster.redToaster(lcl.CLASSIFICATION_NOT_FOUND_MSG);
//				} else if (error.includes("Folder is associated to a Storage Center with save type")) {
//					this.toaster.redToaster(lcl.TYPE_NOT_COMPAITIBLE)
//				}
//				console.log("Error finding Folder '" + folderNum + "'", resultSet.result);
//			}
//		},
...
//				contentSourceParameters.folderId = this.folderNumberField.get("value");
//				contentSourceParameters.numberOfPages = this.numberOfPagesField.get("value");
				contentSourceParameters.sourceId = "2";
...
//						}),null,null,onError,true);
		...
		onAdd: function() {
			...
			// CUSTOMIZATION
//			var isPhysical = this.PhysicalArchiveCheckBox.get("value");
//			if (isPhysical == "on") {
//				var folderId = this.folderNumberField.get("value");
//				if (!folderId || folderId.trim() == "") {
//					this.toaster.redToaster(lcl.PHYSICAL_FOLDER_NUM_REQUIRED);
//					return;
//				}
//				
//				var numOfPages = this.numberOfPagesField.get("value");
//				if (!numOfPages || numOfPages.trim() == "") {
//					this.toaster.redToaster(lcl.PAGES_NUM_REQUIRED);
//					return;
//				}
//			}
...
//					this.addContentItemGeneralPane._contentTable.insertBefore(fileCategory, this.addContentItemGeneralPane._contentTable.children[0]);
		...
		_setControlStateDefaults: function() {
			...
			// AbdelRahman Code:
			// test disable div
//			this.folderNumberField.set("disabled", true);
//			this.numberOfPagesField.set("disabled", true);
		},

		/**
		 * PhysicalArchive CheckBox func
		 */
//		checkPhysicalArchive: function() {
//			if (this.PhysicalArchiveCheckBox.checked){
//				this.folderNumberField.set("disabled", false);
//				this.numberOfPagesField.set("disabled", false);
//			} else {
//				this.folderNumberField.set("disabled", true);
//				this.numberOfPagesField.set("disabled", true);				
//			}
//		},
		 ...
		 * @private
		 */
		_validateEDS: function(file, isDocVisible, callback) {
			var addAsMinorVersion = !this.addContentItemGeneralPane.getAsMajorVersion();
			var autoClassify = this.addContentItemGeneralPane.getAutoClassify();
			var compoundDocument = this._entryTemplate && this._entryTemplate.enableCompoundDocuments && this._entryTemplate.enableCompoundDocuments.on ? true : false;
			var documentType = this.addContentItemPropertiesPane.getDocumentType();
			...
			if (this._typeDocument) {
				...
				var contentSourceType = this.addContentItemGeneralPane.getContentSourceType();
				var uploadViaAspera;
				if (contentSourceType == "Document" && ecm.model.desktop.enableAspera && ecm.model.desktop.AsperaConnectInstalled && this.repository._isP8()){
					uploadViaAspera = true;
				}
				if (contentSourceType == "Document" || contentSourceType == "RepositoryDocument") {
					if (file.filename) {
						file.fileName = file.filename;
					}
					...
					// Set the file name to the Title property for EDS validation 
					// This is the case of "apply-same-properties-to-all-documents" option or the case where the user did not visit all pages
					// and then click Add. 
					// Need to validate the implied document title as it is potentially different for each document.
					if (this.addContentItemPropertiesPane.isApplySamePropsToAllDocs() || !isDocVisible) {
						var titlePropertyName = this.addContentItemPropertiesPane.getTitlePropertyName();
						array.some(properties, lang.hitch(this, function(property) {
							if (property.name == titlePropertyName) {
				...
				var contentSourceParameters;
				// Set the isPreFlightCheck parameter to do just the EDS validation only
				contentSourceParameters = {
					"preFlightCheck" : "true"
				};
				
				if (documentItem.contentSourceType == "RepositoryDocument") {
					lang.mixin(contentSourceParameters, {
						"contentSourceRepositoryId": documentItem.contentSourceRepositoryId,
						"contentSourceDocId": documentItem.contentSourceDocId,
						"contentSourceDelete": documentItem.contentSourceDelete
					});
				...
				// call addDocumentItem request but pass in preFlightCheck parameter in contentSourceParameters to do just the EDS validation only
				request = this.repository.addDocumentItem(parentFolder, objectStore, documentItem.templateName, documentItem.criterias, 
						documentItem.contentSourceType, documentItem.mimetype, documentItem.filename, documentItem.content, documentItem.childComponentValues, 
						documentItem.permissions, documentItem.securityPolicyId, documentItem.addAsMinorVersion, documentItem.autoClassify, 
						documentItem.allowDuplicateFileNames, documentItem.setSecurityParent, documentItem["teamspaceId"], 
						lang.hitch(this, this._validateEDSCallback, isDocVisible, callback), isBackgroundRequest, onError, documentItem.compoundDocument, 
						uploadProgress, documentItem.application_group, documentItem.application, contentSourceParameters, 
						documentItem.templateMetadataValues, documentItem.fullPath);
		 ...
		 * @private
		 */
		_validateEDSCallback: function(isDocVisible, callback, response) {
			var edsValidationSuccessful = response.edsValidationSuccessful;
			var fieldErrors = response.fieldErrors;
			var templateMetadataErrors = response.templateMetadataErrors;

			if (edsValidationSuccessful) {
				callback(null);
			} else {
				var invalidProps = null;
				var invalidTitlePropOnly = fieldErrors != null && fieldErrors.length == 1 && fieldErrors[0].symbolicName == this.addContentItemPropertiesPane.getTitlePropertyName();
				// Only apply errors to the form if we're validating the document being displayed (isDocVisible) 
				// Special case: if the option is apply-to-all, we only want to apply errors if there are invalid properties other than the title because the title property is read-only and does not display the file name in this case
				if (this.addContentItemPropertiesPane.isApplySamePropsToAllDocs() ? (isDocVisible && !invalidTitlePropOnly) : isDocVisible) {
					// set focus on fields that have errors
					this._handleFieldErrors(fieldErrors, templateMetadataErrors);
				}
				if (fieldErrors && fieldErrors.length) {
					var invalidProps = [];
					for (var i = 0; i < fieldErrors.length; i++) {
						invalidProps.push(fieldErrors[i].symbolicName);
					}
				}
				callback(invalidProps);
			}
		},
		/**
		 * @private
		 */
		_handleFieldErrors: function(fieldErrors, templateMetadataErrors) {
			if (fieldErrors) {
				var fieldErrorMap = {};
				array.forEach(fieldErrors, function(fieldError) {
					this.addContentItemPropertiesPane.setFieldError(fieldError.symbolicName, fieldError.errorMessage, fieldError.invalidItems);
